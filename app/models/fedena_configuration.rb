...
class FedenaConfiguration < ActiveRecord::Base
  ...
  def validate
    if self.config_key == "StudentAttendanceType"
      errors.add_to_base("#{I18n.t('student_attendance_type_should_be_one')} #{STUDENT_ATTENDANCE_TYPE_OPTIONS}") unless FedenaConfiguration::STUDENT_ATTENDANCE_TYPE_OPTIONS.collect{|d| d[1] == self.config_value}.include?(true)
    end
...
#   FedenaConfiguration table entries
#
